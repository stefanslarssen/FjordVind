...
import { fetchMortality, fetchCages, MortalityRecord, Merd } from '../services/api';
...
export default function DodlighetOversiktScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      // Get all merds
      const merdsData = await fetchCages();

      // Get mortality records for period
      const mortalityData = await fetchMortality({ fromDate: startDateStr });

      // Calculate totals per merd
      const totalsMap: Record<string, number> = {};
      mortalityData.forEach((r) => {
        totalsMap[r.merd_id] = (totalsMap[r.merd_id] || 0) + (r.antall_dod || 0);
      });

      // Create summaries for all merds
      const summaryList: MerdSummary[] = merdsData.map((m) => ({
        merd_id: m.id,
      ...
      mortalityData.forEach((r: any) => {
        ...
        if (dailyMap[dateStr]) {
          dailyMap[dateStr].total += r.antall_dod || 0;
